    ...
    if (result.success) {
      ...
      navigate(
        loggedUser?.restaurantId
          ? `/dashboard/restaurants/${loggedUser.restaurantId}`
          : '/dashboard'
      );
        ...
        >
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end">
              <Link
                to="/forgot-password"
                className="text-[9px] font-black uppercase tracking-widest text-[#b98c52] hover:underline"
              >
                ¿Olvidaste tu acceso?
              </Link>
            </div>
          ...
          <div className="mt-10 pt-8 border-t-2 border-[#1c1712]/10 flex flex-col items-center gap-4">
            ...
            <Link
              to="/register"
              className="text-[10px] font-black uppercase tracking-widest text-[#1c1712] px-6 py-2 border-2 border-[#1c1712] shadow-[4px_4px_0px_#b98c52] hover:-translate-y-1 transition-all"
            >
              Registrar Cuenta
            </Link>
          </div>
        ...
        >
          ...
          <div className="absolute bottom-10 right-10 flex gap-3 z-30">
            {uploadImages.map((_, i) => (
              <div
                key={i}
                className={`h-1.5 border border-[#1c1712] transition-all duration-500 ${i === currentImage
                    ? 'w-12 bg-[#b98c52]'
                    : 'w-3 bg-white/30'
                  }`}
              />
